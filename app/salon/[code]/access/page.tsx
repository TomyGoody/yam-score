...
export default function PlayerAccessPage() {
      ...
      .select(`
        ...
        if (salonData.competition_id) {
          ...
          if (competitionType === "grand_prix") {
            ...
            .select("circuit_id")
            .eq("competition_id", salonData.competition_id)
            .eq("game_id", salonData.id)
            .maybeSingle();
            
            circuitId = grandPrix?.circuit_id ?? null;
          }
        }
    ...
    async function startGame() {
      ...
      if (error) {
        ...
        setErrorMessage(
          ...
          ? "Impossible de démarrer le match."
          : salon.competition_type === "grand_prix"
          ? "Impossible de démarrer le Grand Prix."
          : "Impossible de démarrer le set."
        );
        return;
      }
  ...
  salon.competition_type === "grand_prix"
  ? getGrandPrixCircuitTheme(salon.circuit_id)
  : salon.competition_theme
  ...
  const themeButtonBackground =
  getThemeColor(tournamentTheme?.buttonBackground, "bg") ?? "#C44934";
  
  const themeButtonText =
  getThemeColor(tournamentTheme?.buttonText, "text") ?? "#FFFFFF";
  
  const themeBorder =
  getThemeColor(tournamentTheme?.border, "border") ?? "#9B6A28";
  ...
  return (
    ...
    onClick={() => {
      if (salon.competition_id) {
        if (salon.competition_type === "world_cup") {
          router.push(
        ...
        if (salon.competition_type === "grand_prix") {
          router.push(
            `/modes-speciaux/grand-prix/${salon.competition_id}`
          );
          return;
        }
  ...
  Joueur {player.player_order}
              ...
              {player.profile_id
                ...
                {isDev
                  ...
                  className="w-full max-w-md rounded-xl border px-5 py-4 text-lg font-black transition disabled:cursor-not-allowed disabled:opacity-40"
                  style={{
                    backgroundColor: themeButtonBackground,
                    color: themeButtonText,
                    borderColor: themeBorder,
